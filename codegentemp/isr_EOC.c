...
{
    CyRamVectors[CYINT_IRQ_BASE + isr_EOC__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + isr_EOC__INTC_NUMBER];
}
...
void isr_EOC_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((isr_EOC__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *isr_EOC_INTC_PRIOR = (*isr_EOC_INTC_PRIOR & (uint32)(~isr_EOC__INTC_PRIOR_MASK)) |
...
uint8 isr_EOC_GetPriority(void)
{
    ...
    priority = (*isr_EOC_INTC_PRIOR & isr_EOC__INTC_PRIOR_MASK) >> priorityOffset;
